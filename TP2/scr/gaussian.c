...
void  gaussian_filter(double ** entree_re,double **entree_im ,int nl,int nc){
    for(int i = 0 ; i < nl ; i++){
	for(int j = 0 ; j < nc ; j++){
	    entree_re[i][j] *= fft_gaussian(2,i,j,nl,nc);
	    entree_im[i][j] *= fft_gaussian(2,i,j,nl,nc);
	}
    }
}
